...
class gsp_nest5Empresas_companycombos_gsp extends GroovyPage {
public String getGroovyPageFileName() { "/WEB-INF/grails-app/views/company/combos.gsp" }
public Object run() {
Writer out = getOut()
...
printHtmlPart(16)
if(true && (element?.fields?.cost / (element?.fields?.price != "NaN" ? element?.fields?.price : 1) >=1)) {
printHtmlPart(17)
}
printHtmlPart(18)
...
printHtmlPart(21)
invokeTag('formatNumber','g',94,['number':(element?.fields?.cost / (((element?.fields?.price != "NaN") && (elemen?.fields?.price != 0)) ? element?.fields?.price : 1)),'type':("number"),'maxFractionDigits':("2")],-1)
printHtmlPart(22)
expressionOut.print(createLink(controller: 'company',action: 'editCombo',params: [id: element?.syncId]))
printHtmlPart(23)
i++
}
}
...
public static final long LAST_MODIFIED = 1402794527441L
